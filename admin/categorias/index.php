<?php
/**
 * admin/categorias/index.php — Listado y creación rápida de categorías de noticias.
 */
...
$pageTitle = 'Categorías de Noticias';
$basePath  = '../../';
$pdo       = get_pdo();
$errors    = [];
$msg       = $_GET['msg'] ?? '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'crear') {
    ...
    if (!$nombre) {
        ...
        // Verificar unicidad
        $stmtChk = $pdo->prepare("SELECT COUNT(*) FROM categorias_noticias WHERE slug = ? OR nombre = ?");
        $stmtChk->execute([$slug, $nombre]);
        if ($stmtChk->fetchColumn() > 0) {
            $errors[] = 'Ya existe una categoría con ese nombre o slug.';
        } else {
            $pdo->prepare("INSERT INTO categorias_noticias (nombre, slug) VALUES (?, ?)")
                ->execute([$nombre, $slug]);
...
$categorias = $pdo->query("SELECT * FROM categorias_noticias ORDER BY nombre")->fetchAll();
...
<div class="row g-3">
  ...
  <div class="col-lg-8">
    <div class="adm-card">
      <h2 class="adm-card-title">Categorías registradas</h2>
      <?php if ($categorias): ?>
                    ...
                    <div class="d-flex gap-2">
                      ...
                      <form method="POST" action="eliminar.php" style="display:inline">
                        ...
                        <button type="submit" class="btn btn-sm btn-adm-danger btn-delete-confirm"
                                ...
                                data-confirm="¿Eliminar la categoría «<?= htmlspecialchars($cat['nombre'], ENT_QUOTES) ?>»? Las noticias asociadas no se borrarán, pero se quedarán sin categoría.">
    ...
    <div class="adm-card">
      ...
      <form method="POST">
        ...
        <div class="mb-3">
          ...
          <input type="text" class="form-control" id="nombre" name="nombre" required placeholder="Ej: Astrofísica">
        </div>
